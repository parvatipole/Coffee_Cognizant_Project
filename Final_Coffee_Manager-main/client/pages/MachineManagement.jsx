...
export default function MachineManagement({ officePath } = {}) {
  ...
  const getOfficeMachineData = () => {
    ...
    const machineData =
      officeMachines[targetOffice] || officeMachines["Hinjewadi IT Park"];
  ...
  useEffect(() => {
    const loadMachineData = async () => {
      ...
      try {
        let mData = dataManager.getMachine(machineId);

        if (mData) {
          console.log(
            "✅ MACHINE DETAIL: Loaded machine from storage:",
            mData.id,
            "status:",
            mData.status,
          );
          ...
          try {
            ...
            if (apiData) {
              mData = dataManager.mapBackendToFrontend(apiData);
              console.log("📡 MACHINE DETAIL: Loaded machine from API:", mData);
              dataManager.saveMachine(mData);
            }
          } catch (apiError) {
          ...
          if (!mData) {
            mData = getMachineDataById(machineId);
            if (mData) {
              console.log(
                "💾 MACHINE DETAIL: Using static fallback data for:",
                machineId,
              );
              dataManager.saveMachine(mData);
            }
        ...
        if (mData) {
          ...
          const dynamicAlerts = generateDynamicAlerts(mData);
          const existingAlerts = mData.alerts || [];
    ...
    const handleStorageChange = (e) => {
      if (e.key === "coffee_shared_machines" && e.newValue !== e.oldValue) {
        console.log(
          "🔄 MACHINE DETAIL: Detected storage change, reloading machine data...",
        );
        loadMachineData();
      }
    };
    ...
    const handleMachineDataChanged = (e) => {
      if (
        e.detail?.machine?.id === machineId ||
        e.detail?.machine?.machineId === machineId
      ) {
        console.log(
          "🔄 MACHINE DETAIL: Machine data changed event received, reloading...",
        );
        loadMachineData();
      }
    };
  ...
  const handleElectricityStatusChange = async (newElectricityStatus) => {
    ...
    console.log(
      `🔒 PERMANENT CHANGE: Machine ${machineData.id} electricity status changed to "${newElectricityStatus}" (machine status: "${newStatus}") - Admin will see this!`,
    );
  ...
  const handleSupplyRefill = async (supplyKey, amount) => {
    ...
    const updatedRefills = [newRefill, ...(machineData.recentRefills || [])].slice(0, 10);
    ...
    try {
      ...
      console.log("Backend unavailable, saved supplies locally only:", error.message);
    }
  };
  ...
  const handlePowerStatusChange = async (newStatus) => {
    ...
    dataManager.saveMachine(updatedData);
    console.log(
      `🔒 PERMANENT CHANGE: Machine ${machineData.id} power status changed to "${newStatus}" - Admin will see this!`,
    );
            ...
            <InteractiveBreadcrumb
              backUrl={
                user?.role === "technician"
                  ...
                  ? `/office/${officeNameToPath(
                      machineData.office || pathToOfficeName(officePath || ""),
                    )}`
                  : "/dashboard"
              }
              className="flex-1"
                      ...
                      return (
                        <Alert key={alert.id} className={getAlertColor(alert.type, alert.resolved)}>
                          ...
                          <AlertDescription>
                            <div className="flex items-center justify-between">
                              <div>
                                <strong>{getAlertPrefix(alert.type, alert.resolved)}</strong>{" "}
                              ...
                              {canEdit && !alert.resolved && (
                                <div className="flex items-center gap-2 ml-4">
                                  <Button size="sm" variant="outline" onClick={() => handleAlertResolution(alert.id)}>
            ...
            <TabsContent value="supplies" className="space-y-6">
              <Card className="animate-fadeIn">
                <CardHeader>
                  ...
                  <CardDescription>Current supply levels and consumption trends</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {supplies.map((supply) => (
                      <div key={supply.key} className="text-center">
                        <div className="flex items-center justify-center mb-2">{supply.icon}</div>
                  ...
                  >
                    <CardHeader className="pb-3">
                      <CardTitle className="flex items-center justify-between text-base">
                        <div className="flex items-center gap-2">{supply.icon}{supply.name}</div>
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <Progress value={supply.current} className="h-3 transition-all duration-1000 ease-out" />
                      ...
                      {canEdit && (
                        <Button size="sm" variant="outline" className="w-full group hover:bg-primary hover:text-primary-foreground transition-colors">
                ...
                <CardContent>
                  <div className="space-y-3">
                    {machineData.recentRefills && machineData.recentRefills.length > 0 ? (
                      machineData.recentRefills.map((refill) => {
                        ...
                        return (
                          <div key={refill.id} className={`flex items-center justify-between p-3 rounded-lg border ${getBgColor(refill.supplyType)}`}>
                            <div>
                            ...
                            <div className="text-right">
                              ...
                              <div className="text-xs text-muted-foreground">by {refill.technician}</div>
                            </div>
                          </div>
                        );
                      })
